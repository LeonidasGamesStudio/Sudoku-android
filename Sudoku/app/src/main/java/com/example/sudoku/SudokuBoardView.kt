...
class SudokuBoardView (context: Context, attributeSet: AttributeSet) : View(context,attributeSet){
    ...
    var sudokuNumbers = Array(9) {Array(9) {0} }
    ...
    fun addNumberToMatrix(number: Int){
        ...
        sudokuNumbers[selectedRow][selectedColumn] = number
    ...
    private fun checkBoardConflicts(){
        for (j in 0 until size){        //iterate through rows
            for (i in 0 until size){    //iterate through columns
                if (checkCellConflicts(i, j, kotlin.math.abs(sudokuNumbers[i][j]))){  //check for conflicts with specific cell
                    sudokuNumbers[i][j] = kotlin.math.abs(sudokuNumbers[i][j]) * -1
                }else{
                    sudokuNumbers[i][j] = kotlin.math.abs(sudokuNumbers[i][j])
                }
            }
        }
    }
    ...
    private fun checkRow(row: Int, column: Int, number: Int): Boolean {
        for (i in 0 until size){
            if(i != column){
                if (kotlin.math.abs(sudokuNumbers[row][i]) == number){
                    return true
                }
            }
        }
    ...
    private fun checkColumn(row: Int, column: Int, number: Int): Boolean {
        for (i in 0 until size){
            if (i != row){
                if (kotlin.math.abs(sudokuNumbers[i][column]) == number){
                    return true
                }
            }
        }
    ...
    private fun checkSquare(row: Int, column: Int, number: Int): Boolean {
        ...
        for (i in squareX..squareX + 2){
            for (j in squareY..squareY + 2){
                if (i != row || j != column){
                    if (kotlin.math.abs(sudokuNumbers[i][j]) == number){
                        return true
                    }
                }
            }
        ...
        for (i in 0 until size){
            for (j in 0 until size){
                if(sudokuNumbers[i][j] > 0){
                    canvas.drawText(sudokuNumbers[i][j].toString(), j * cellSizePixels + (cellSizePixels / 2), i * cellSizePixels + (cellSizePixels / 2) - yOffset, numberPaint)
                }else if(sudokuNumbers[i][j] < 0){
                    canvas.drawText((sudokuNumbers[i][j] * -1).toString(), j * cellSizePixels + (cellSizePixels / 2), i * cellSizePixels + (cellSizePixels / 2) - yOffset, conflictPaint)
                }
            }
        }
    }
